...
class AttentionQKV(Model):
    ...
    def call(self, queries, keys, values, mask=None):
        ...
        similarity = tf.matmul(queries, keys, transpose_b=True) / tf.math.sqrt(key_dim) ##b, h, n_q, n_keyval
        ...
        output = tf.matmul(weights, values) ##b, h, n_q, n_depth_v
...
class MultiHeadProjection(Model):
    ...
    def _split_heads(self, tensor):
        ...
        tensor = tf.transpose(tensor, perm=[0, 2, 1, 3]) #(b, h, l, nf)
        return tensor
        ##########################################################################################
    ...
    def _combine_heads(self, tensor):
        ...
        new_feature_size = int(self.n_heads*feature_size) # What is the new feature size, if we combine all the heads
