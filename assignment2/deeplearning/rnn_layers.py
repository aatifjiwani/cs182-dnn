...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    X_H_bias = np.add(X_H, b)
    next_h = np.tanh(X_H_bias)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = np.empty((N, T, H))
    for t in range(T):
        next_h, t_cache = rnn_step_forward(x[:, t, :], prev_h, Wx, Wh, b)
        h[:, t, :] = next_h
        prev_h = next_h

    return h, (x, h0, Wx, Wh, b, h)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        ...
        curr_cache = (x[:,t,:], prev_h, Wx, Wh, b, h[:,t,:])
        
        dx_t, dprev_h_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:,t,:] + dprev_h, curr_cache)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (i, f, o, g, x, Wx, Wh, prev_c, next_c, prev_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    i, f, o, g, x, Wx, Wh, prev_c, next_c, prev_h = cache
...
def lstm_backward(dh, cache):
    ...
    """
    dx, dh0, dWx, dWh, db = None, None, None, None, None
    i, f, o, g, x, Wx, Wh, prev_c, next_c, prev_h = cache[0]
    
    N,T,H = dh.shape
    _,D = x.shape
    
    #print("N", N,"T", T,"H", H,"D",D)
    ...
    for t in range(T-1, -1, -1):        
        ...
        dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:, t, :] + dprev_h, dnext_c, curr_cache)
        #print(dWx_t.shape)
