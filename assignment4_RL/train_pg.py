...
class Agent(object):
    ...
    def define_placeholders(self):
        """
        Placeholders for batch observations / actions / advantages in
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            sy_logstd = tf.get_variable(name="stddev", shape=self.ac_dim, dtype=tf.float32)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # START OF YOUR CODE
            # ------------------------------------------------------------------
            z = tfd.MultivariateNormalDiag(loc=tf.zeros_like(sy_mean), scale_diag=tf.ones(sy_logstd.shape))
            sy_sampled_ac = sy_mean + tf.exp(sy_logstd) * z.sample(shape=sy_mean.shape)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            # START OF YOUR CODE
            # ------------------------------------------------------------------
            sy_ac_na = (sy_ac_na - sy_mean) / tf.exp(sy_logstd)
            sy_logprob_n = tfd.MultivariateNormalDiag(loc=tf.zeros_like(sy_mean), scale_diag=tf.ones(sy_logstd.shape)).log_prob(sy_ac_na)
        ...
        if self.reward_to_go:
            ...
            for path in re_n:
                ...
                for i in range(len_path):
                    Q_t = np.sum( np.array([self.gamma**t for t in range(len_path-i)])*path[i:] )
                    q_n = np.append(q_n, Q_t)
        else:
            for path in re_n:
                len_path = path.shape[0]
                
                gammas = np.array([self.gamma**i for i in range(len_path)])
                total_reward = np.sum(gammas * path)
                q_n += [total_reward]*len_path
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        prev_loss, _ = self.sess.run([self.loss, self.update_op], feed_dict=feed_dict)
        curr_loss = self.sess.run(self.loss, feed_dict=feed_dict)
        
        print("Previous loss: ", prev_loss, "Current loss: ", curr_loss)
        # ------------------------------------------------------------------
        # END OF YOUR CODE
        # ------------------------------------------------------------------
