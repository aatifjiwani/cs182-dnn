...
class QLearner(object):
    ...
    def __init__(self,
        ...
        self.target_q_tp1 = q_func(obs_tp1_float, self.num_actions, scope="target_q_func", reuse=False)
        ...
        if self.double_q:
            q_base_tp1 = q_func(obs_tp1_float, self.num_actions, scope="base_q_func", reuse=False)
            best_actions = tf.argmax(q_base_tp1, axis=1)
            tp1_qvals = tf.reduce_sum(self.target_q_tp1*tf.one_hot(best_actions, self.num_actions), axis=1)
        else:
            tp1_qvals = tf.reduce_max(self.target_q_tp1, axis=1)
    ...
    def step_env(self):
        ...
        if not self.model_initialized or random.random() < epsilon:
            ...
            actions = self.session.run(tf.argmax(self.base_q_t, axis=1), feed_dict={
                self.obs_t_ph: np.expand_dims(encoded_obs, axis=0)
            })
            action_tp1 = actions[0]
        ...
        self.replay_buffer.store_effect(self.replay_buffer_idx, action_tp1, reward, done)
        self.last_obs = obs
