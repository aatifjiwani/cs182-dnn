...
class QLearner(object):
    ...
    def __init__(self,
        ...
        self.base_q_t = q_func(obs_t_float, self.num_actions, scope="base_q_func", reuse=False)
        target_q_tp1 = q_func(obs_tp1_float, self.num_actions, scope="target_q_func", reuse=False)
        
        q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='base_q_func')
        target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="target_q_func")
        
        t_qvals = tf.reduce_sum(self.base_q_t * tf.one_hot(self.act_t_ph, self.num_actions), axis=1)
        
        if self.double_q:
            q_base_tp1 = q_func(obs_tp1_float, self.num_actions, scope="base_q_func", reuse=True)
            best_actions = tf.argmax(q_base_tp1, axis=1)
            tp1_qvals = tf.reduce_sum(target_q_tp1*tf.one_hot(best_actions, self.num_actions), axis=1)
        else:
            tp1_qvals = tf.reduce_max(target_q_tp1, axis=1)
        
        
        error = self.rew_t_ph + gamma * (1 - self.done_mask_ph) * tp1_qvals - t_qvals
    ...
    def step_env(self):
        ...
        curr_idx = self.replay_buffer.store_frame(self.last_obs)
        encoded_obs = self.replay_buffer.encode_recent_observation()
        
        if not self.model_initialized or random.random() < epsilon:
            action_tp1 = self.env.action_space.sample()
        else:
            actions = self.session.run(self.base_q_t, feed_dict={
                self.obs_t_ph: np.expand_dims(encoded_obs, axis=0)
            })
            action_tp1 = np.argmax(actions, axis=1)[0]
        
        
        obs, reward, done, info = self.env.step(action_tp1)
        if done:
            obs = self.env.reset()
        
        self.replay_buffer.store_effect(curr_idx, action_tp1, reward, done)
        self.last_obs = obs
        #rint("end stepping")
        # ----------------------------------------------------------------------
        # END OF YOUR CODE
